...
std::string mainProcess(char const *inputStr, RequestType requestType) {
  ...
  switch (requestType) {
    ...
    case EXPLAIN_TOP_MOVES: {
      return explainTopMoveList(startingGameState, curPiece, nextPiece, NUM_TOP_ENGINE_MOVES, playoutCount, playoutLength, &context, pieceRangeContextLookup);
    }
